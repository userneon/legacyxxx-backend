...
The frontend understands rank image keys `rank-01` through `rank-18` and has a numeric fallback so missing artwork does not render as a broken image. Initial `ranks/1.png`–`18.png` assets were identified as Counter-Strike profile-rank art rather than the user's horizontal competitive matchmaking reference. The correct game-binary extracted source is `ItzArty/csgo-rank-icons/matchmaking/1.svg` through `18.svg`. The source set was rasterized into individual 256×256 RGBA PNG files without semantic alteration for the requested frontend format; visual verification confirmed Rank 1 as the Silver I chevron tile and Rank 18 as the Global Elite globe-and-wings tile. These assets supersede the incorrect raster sequence. The new plugin source is built locally but is intentionally not pushed until explicit plugin-repository push authorization is reconfirmed; no game server deployment was attempted.
